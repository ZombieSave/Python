...
def int_func(word):
    ...
    if all(map(lambda x: 97 <= ord(x) <= 122, word)):
        result = chr(ord(word[0:1]) - 32) + word[1:] + " "

    return result


input_str = input("Введите строку из маленьких латинских букв: ")
array = input_str.split()
result = "".join(map(int_func, array))
print(result)
